...
const KODE_FILE = 'SBM.PD-1-50-2025';
...
const INVOICE_TARGETS = [
    { stage: 1, label: 'Tahap pertama', invoiceDate: '2025-12-02' },
    { stage: 2, label: 'Tahap kedua', invoiceDate: '2026-01-02' },
    { stage: 3, label: 'Tahap ketiga', invoiceDate: '2026-08-03' }
];

const PAYMENT_TARGETS = [
    { stage: 1, label: 'Tahap pertama', paymentDate: '2025-12-19' },
    { stage: 2, label: 'Tahap kedua', paymentDate: '2026-03-16' }
];
...
function nextInvoiceNumber(dateValue, rows, reserved) {
    ...
    rows.forEach(row => {
        ...
        if (sequence && suffix && Number(suffix[1]) === year) max = Math.max(max, Number(sequence[1]) || 0);
    });
    max = Math.max(max, Number(reserved.get(key) || 0));
    const sequence = max + 1;
    reserved.set(key, sequence);
...
async function main() {
    ...
    const allTargets = [...INVOICE_TARGETS, ...PAYMENT_TARGETS];
    const terms = allTargets
        .filter((target, index, targets) => targets.findIndex(item => item.stage === target.stage) === index)
        .map(target => {
    ...
    const invoicePlans = INVOICE_TARGETS.map(target => {
        const item = terms.find(term => term.target.stage === target.stage);
        const existing = findUniqueStage(codeInvoices, text(program.id_program), target, 'Invoice');
        return {
            ...item,
            target,
            existing,
            invoiceNumber: existing
                ? text(existing.nomor_invoice)
                : nextInvoiceNumber(target.invoiceDate, allInvoices, invoiceNumberReserved)
        };
    });

    const selectedPaymentIds = new Set();
    const paymentPlans = PAYMENT_TARGETS.map(target => {
        const item = terms.find(term => term.target.stage === target.stage);
        const existing = findUniqueStage(codePayments, text(program.id_program), target, 'Pembayaran');
        if (existing) selectedPaymentIds.add(text(existing._id));
        const keepExistingBkm = existing
            && isTrue(existing.bkm_sudah_disimpan)
            && text(existing.bkm_nomor)
            && normalizeDate(existing.bkm_tanggal || existing.tanggal) === target.paymentDate;
        return {
            ...item,
            target,
            existing,
            bkmNumber: keepExistingBkm ? text(existing.bkm_nomor) : ''
        };
    });
    ...
    paymentPlans
        ...
        .forEach(item => {
            if (item.bkmNumber) return;
            item.bkmNumber = nextBkmNumber(
                item.target.paymentDate,
                text(item.existing?.bkm_no_rekening) || DEFAULT_ACCOUNT,
                existingPaymentRows,
                bkmNumberReserved
            );
        });
    ...
    for (const item of paymentPlans) {
        ...
        if (item.existing) {
            ...
            if (isTrue(item.existing.bkm_sudah_disimpan)) {
            ...
            paymentResults.push({
                ...
                id: text(item.existing._id),
                tanggal: item.target.paymentDate,
                bkm_tanggal: update.bkm_tanggal || normalizeDate(item.existing.bkm_tanggal),
                bkm_nomor: update.bkm_nomor || text(item.existing.bkm_nomor)
            });
            continue;
        }

        const nominalNetto = Math.round(nominalPks * 0.8);
        const keterangan = `Realisasi ${item.term.label || item.target.label} dari rencana penerimaan`;
