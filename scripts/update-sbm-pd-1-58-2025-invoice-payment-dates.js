...
const KODE_FILE = 'SBM.PD-1-58-2025';
...
const INVOICE_TARGETS = [
    { stage: 1, label: 'Tahap pertama', invoiceDate: '2026-01-19' },
    { stage: 2, label: 'Tahap kedua', invoiceDate: '2026-01-30' },
    { stage: 3, label: 'Tahap ketiga', invoiceDate: '2026-08-21' }
];

const PAYMENT_TARGETS = [
    { stage: 1, label: 'Tahap pertama', paymentDate: '2026-01-20' },
    { stage: 2, label: 'Tahap kedua', paymentDate: '2026-02-11' }
];
...
async function main() {
    ...
    const terms = INVOICE_TARGETS.map(target => {
    ...
    const invoicePlans = terms.map(item => {
        const existing = findUniqueStage(codeInvoices, text(program.id_program), item.target, 'Invoice');
        return {
            ...item,
            existing,
            invoiceNumber: existing
                ? text(existing.nomor_invoice)
                : nextInvoiceNumber(item.target.invoiceDate, allInvoices, invoiceNumberReserved)
        };
    });
    ...
    const paymentPlans = PAYMENT_TARGETS.map(target => {
        ...
        return { target, existing, bkmNumber: '' };
    });
    ...
    const plan = {
        ...
        payments: paymentPlans.map(item => ({
            ...
            nominal_pks: Number(item.existing?.rencana_nominal) || 0,
            tindakan: 'update'
        }))
    };
